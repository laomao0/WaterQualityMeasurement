...
{ 	
 	...
 	RCC_APB2PeriphClockCmd(SLC_SDA_RCC, ENABLE);	 //Ê¹ÄÜPC,D,G¶Ë¿ÚÊ±ÖÓ

	GPIO_InitStructure.GPIO_Pin = SCL_GPIO_PIN|SDA_GPIO_PIN;	 //PD3,PD6ÍÆÍìÊä³ö  
 	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; 		 //ÍÆÍìÊä³ö
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;//ËÙ¶È50MHz
 	GPIO_Init(SLC_SDA_PORT, &GPIO_InitStructure);	  //³õÊ¼»¯GPIOD3,6
 	GPIO_SetBits(SLC_SDA_PORT,SCL_GPIO_PIN|SDA_GPIO_PIN);	//PD3,PD6 Êä³ö¸ß
...
{
	OLED_Clear();
	OLED_ShowString(0,0,"    PH ",16);//??? -- 8x16???????ASCII?
	OLED_ShowString(0,2,"    ORP  (mv)",16);
	OLED_ShowString(0,4,"    RC   (ppm)",16);
